...
export default function Cardrender(props) {
    ...
    if (data?.animals.length !== 0) {
        switch (true) {
            case settings.age !== null &&
                settings.size !== null &&
                settings.house_trained !== null &&
                settings.animals !== null: {
                cardData = animals.filter(
                    (animal) =>
                        animal.age === settings.age &&
                        animal.size === settings.size &&
                        animal.house_trained === settings.house_trained &&
                        animal.type === settings.animals
                );
                break;
            }
            ...
            case settings.age !== null: {
                cardData = animals.filter(
                    (animal) => animal.age === settings.age
                );
                break;
            }
            ...
            case settings.size !== null: {
                cardData = animals.filter(
                    (animal) => animal.size === settings.size
                );
                break;
            }
            ...
            case settings.house_trained !== null: {
                cardData = animals.filter(
                    (animal) => animal.house_trained === settings.house_trained
                );
                break;
            }
            ...
            case settings.animals !== null: {
                cardData = animals.filter(
                    (animal) => animal.type === settings.animals
                );
                break;
            }
    ...
    const outOfFrame = async (direction, id) => {
        if (direction === "right") {
            console.log("I'm being swiped right on");
            try {
    ...
    if (cardData.length === 0) {
        return (
            <div>
                No animals found! Change your filter settings to get more
                animals!
            </div>
        );
    }
                    ...
                    return (
                        <TinderCard
                            ...
                            onCardLeftScreen={(direction) =>
                                outOfFrame(direction, animal._id)
                            }
                        >
                            <img
                                src={animal.photo[0].medium}
                                onerror="this.onerror=null; this.src='sheltr_logo2.png'"
                                alt={animal.name}
                                className="swipeImg"
                            />
                            <h2 className="swipeDogName">
                                {animal.name}{" "}
                                <i className="swipeDogAge">{animal.age}</i>
                            </h2>

                            <p className="swipeDogLocation">
                                {animal.organization}
                            </p>
                            <p className="swipeDogOrg">{animal.location}</p>
                        </TinderCard>
                    );
                }
                return (
